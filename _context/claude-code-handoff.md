...
| `src/components/InquiryForm.tsx` | **Contact inquiry form (client component).** "use client" component. 9 fields: Full Name*, Email*, Phone, Date of Service, Passengers (1-55), Pickup Location, Drop-off Location, Vehicle Preference (select with 5 options: Luxury Sedan, Luxury SUV, Luxury Executive Van (up to 14), Motor Coach (26–55 via affiliate), Not sure — help me decide), Message (textarea). On submit: `e.preventDefault()` → sets `submitted` → shows thank you message with 48-hour response time and phone fallback. **CRITICAL: Same as ReservationForm — does NOT actually send data anywhere.** TODO: "Wire to email service (Formspree, Resend, or FASTTRAK)". Contains reusable `Field` sub-component. Slightly different field set from ReservationForm (less required fields, no time field, has message instead of special requests). Vehicle options updated to match current fleet lineup (old options like "Stretch Limousine" and "Passenger Van" removed). |
